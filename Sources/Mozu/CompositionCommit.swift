...
///   注入**して IME 自身に確定させる。
///
/// 英数/かなは同じ入力ソース内のトグルで、その状態は入力ソース ID に
/// 反映されない。外部でどう変わっても（Caps Lock 英数、mozu 非起動中の
/// 英数キーなど）観測のしようがないので、状態を追跡するやめ方自体を
/// 捨てた。復帰は「日本語ソースを選ぶたび必ず `restoreKanaMode()` を
/// 撃つ」でやる。かなキーはかなモードでは IME が消費するだけの実質
/// no-op なので、無条件で安全。
///
